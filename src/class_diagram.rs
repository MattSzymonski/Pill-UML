...
use crate::common::{DiagramStyle, SvgBuilder, escape_xml};
...
impl Parser {
    ...
    fn parse_line(&mut self, line: &str) {
        ...
        if line.starts_with("class ") || line.starts_with("interface ") 
            || line.starts_with("abstract ") || line.starts_with("enum ") {
    ...
    fn parse_class_decl(&mut self, line: &str) {
        ...
        let name_part = if has_body { rest[..rest.len()-1].trim() } else { rest };
        
        // Parse stereotype <<...>>
        let (name, stereotype) = if let Some(start) = name_part.find("<<") {
            if let Some(end) = name_part.find(">>") {
                (name_part[..start].trim().to_string(), Some(name_part[start+2..end].trim().to_string()))
            } else {
    ...
    fn parse_field(&mut self, class_name: &str, visibility: Option<Visibility>, rest: &str) {
        ...
        let (name, field_type) = if let Some(pos) = rest.find(':') {
            (rest[..pos].trim().to_string(), Some(rest[pos+1..].trim().to_string()))
        } else {
            (rest.to_string(), None)
        };
        
        if let Some(class) = self.classes.get_mut(class_name) {
            class.fields.push(Field { visibility, name, field_type, is_static });
        }
    }
        ...
        for (pattern, rel_type) in patterns {
            if let Some(pos) = line.find(pattern) {
                ...
                let (right, label) = if let Some(colon) = right_part.find(':') {
                    (right_part[..colon].trim(), Some(right_part[colon+1..].trim().to_string()))
                } else {
                    (right_part, None)
                };
                ...
                self.relationships.push(Relationship { from, to, rel_type, label });
                return;
            }
        }
    }
...
impl ClassDiagram {
    ...
    fn calculate_dimensions(&mut self, style: &DiagramStyle) {
        ...
        for class in &mut self.classes {
            ...
            for field in &class.fields {
                ...
                max_width = max_width.max(text.len() as f32 * style.char_width + style.padding * 2.0);
            }
            ...
            let fields_h = if class.fields.is_empty() { 0.0 } 
                else { class.fields.len() as f32 * field_height + style.padding };
            let methods_h = if class.methods.is_empty() { 0.0 } 
                else { class.methods.len() as f32 * field_height + style.padding };
            
            class.height = (compartment_height + fields_h + methods_h).max(compartment_height * 2.0);
        }
    }
    ...
    fn hierarchical_layout(&mut self, style: &DiagramStyle) {
        ...
        for rel in &self.relationships {
            match rel.rel_type {
                RelationType::Inheritance | RelationType::Realization => {
                    children.entry(rel.to.clone()).or_default().push(rel.from.clone());
                }
                RelationType::Composition | RelationType::Aggregation => {
                    children.entry(rel.from.clone()).or_default().push(rel.to.clone());
                }
                _ => {}
            }
        }
        ...
        let roots: Vec<_> = names.iter()
            .filter(|n| !self.relationships.iter().any(|r| 
                (r.rel_type == RelationType::Inheritance || r.rel_type == RelationType::Realization) && &r.from == *n))
            .cloned()
            .collect();
        ...
        while let Some((name, layer)) = queue.pop() {
            if layers.contains_key(&name) { continue; }
        ...
        for layer in layer_list {
            ...
            let max_height = indices.iter().map(|&i| self.classes[i].height).fold(0.0f32, f32::max);
    ...
    fn bounds(&self, style: &DiagramStyle) -> (f32, f32) {
        let max_x = self.classes.iter().map(|c| c.x + c.width).fold(0.0f32, f32::max);
        let max_y = self.classes.iter().map(|c| c.y + c.height).fold(0.0f32, f32::max);
...
fn calculate_path(from: &ClassDef, to: &ClassDef, rel_type: RelationType) -> Vec<(f32, f32)> {
    ...
    let prefer_vertical = matches!(rel_type, RelationType::Inheritance | RelationType::Realization);
    ...
    if dx.abs() > dy.abs() || !prefer_vertical {
        if dx > 0.0 {
            ...
            let route_y = if from_cy > to_cy { from.y.min(to.y) - route_margin } 
                else { (from.y + from.height).max(to.y + to.height) + route_margin };
            return vec![(sx, sy), (sx + route_margin, sy), (sx + route_margin, route_y),
                       (ex - route_margin, route_y), (ex - route_margin, ey), (ex, ey)];
        } else {
