...
impl Parser {
    ...
    fn parse_line(&mut self, line: &str) {
        ...
        if line.starts_with("alt ") {
            self.elements.push(Element::AltStart(line[4..].trim().to_string()));
            return;
        }
...
<polyline points="0 0, 10 3.5, 0 7" fill="none" stroke="{}" stroke-width="1.5"/>
        ...
        style.arrow_color, style.arrow_color
    ));
    ...
    for p in &diagram.participants {
        svg.line(p.x, top_y + participant_height, p.x, bottom_y, &style.lifeline_color, 1.0, true);
    }
    ...
    for elem in &diagram.elements {
        match elem {
            ...
            Element::AltStart(cond) => {
                ...
                // Draw alt header
                svg.push(&format!(
                    r#"<text x="{}" y="{}" font-family="{}" font-size="{}" fill="{}" font-weight="bold">[{}]</text>"#,
                    left_x + 5.0, current_y + 15.0, style.font_family, style.font_size, 
                    style.font_color, escape_xml(cond)
                ));
                current_y += message_spacing;
            }
            Element::ElseBranch(cond) => {
                if let Some(&(_, left_x, right_x)) = alt_stack.last() {
                    ...
                    svg.line(left_x, current_y, right_x, current_y, &style.alt_border_color, 1.0, true);
                    
                    if let Some(c) = cond {
                        svg.push(&format!(
                            r#"<text x="{}" y="{}" font-family="{}" font-size="{}" fill="{}">[{}]</text>"#,
                            left_x + 5.0, current_y + 15.0, style.font_family, style.font_size,
                            style.font_color, escape_xml(c)
                        ));
                    }
                }
            ...
            Element::AltEnd => {
                if let Some((start_y, left_x, right_x)) = alt_stack.pop() {
                    ...
                    svg.push(&format!(
                        ...
                        left_x, start_y, right_x - left_x, box_height, style.alt_border_color
                    ));
                    // Alt label box
                    svg.push(&format!(
                        r#"<polygon points="{},{} {},{} {},{} {},{}" fill="{}" stroke="{}"/>"#,
                        left_x, start_y, left_x + 30.0, start_y, left_x + 40.0, start_y + 15.0,
                        left_x, start_y + 15.0, style.alt_bg_color, style.alt_border_color
                    ));
                    svg.push(&format!(
                        r#"<text x="{}" y="{}" font-family="{}" font-size="10" fill="{}">alt</text>"#,
                        left_x + 5.0, start_y + 11.0, style.font_family, style.font_color
                    ));
                }
                current_y += message_spacing * 0.5;
            }
...
fn draw_participant_box(svg: &mut SvgBuilder, p: &Participant, y: f32, height: f32, style: &DiagramStyle) {
    let x = p.x - p.width / 2.0;
    svg.rect(x, y, p.width, height, &style.participant_bg_color, &style.participant_border_color);
    svg.text_centered(p.x, y + height / 2.0 + 4.0, &p.name, style, false);
}

fn draw_message(svg: &mut SvgBuilder, participants: &[Participant], msg: &Message, y: f32, style: &DiagramStyle) {
    ...
    if msg.from == msg.to {
        ...
        svg.polyline(&points, &style.arrow_color, style.arrow_thickness, dashed, marker);
        
        svg.push(&format!(
            r#"<text x="{}" y="{}" font-family="{}" font-size="{}" fill="{}">{}</text>"#,
            from_p.x + loop_width + 5.0, y + loop_height / 2.0 + 4.0,
            style.font_family, style.font_size, style.font_color, escape_xml(&msg.text)
        ));
    } else {
        // Normal message
        let (x1, x2) = (from_p.x, to_p.x);
        svg.polyline(&[(x1, y), (x2, y)], &style.arrow_color, style.arrow_thickness, dashed, marker);
        
        // Label
        let mid_x = (x1 + x2) / 2.0;
        svg.text_centered(mid_x, y - 5.0, &msg.text, style, false);
    }
}
...
fn draw_divider(svg: &mut SvgBuilder, width: f32, y: f32, text: &str, style: &DiagramStyle) {
    ...
    // Dashed line
    svg.line(left, y, right, y, &style.border_color, 1.0, true);
    
    // Text box in center
    let text_width = text.len() as f32 * style.char_width + 20.0;
    let box_x = (width - text_width) / 2.0;
    
    svg.rect(box_x, y - 10.0, text_width, 20.0, &style.background_color, &style.border_color);
    svg.text_centered(width / 2.0, y + 4.0, text, style, false);
}

fn get_diagram_bounds(participants: &[Participant], style: &DiagramStyle) -> (f32, f32) {
    let left = participants.first()
        .map(|p| p.x - p.width / 2.0 - 10.0)
        .unwrap_or(style.margin);
    let right = participants.last()
        .map(|p| p.x + p.width / 2.0 + 10.0)
        .unwrap_or(200.0);
    (left, right)
}
